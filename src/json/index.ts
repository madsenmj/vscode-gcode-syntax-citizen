...
import { CodeType, CodeTypes } from '../types/codes';
import { MachineType } from '../types/machinetypes';
import { CNCCodes } from '../types/base';
...
export function loadJSON(codeType: CodeType, machineType: MachineType, variant?: Variant): CNCCodes | undefined {
    ...
    switch (codeType) {
        ...
        case CodeTypes.G:
            {
                try {
                    const gFile = fs.readFileSync(path.join(__dirname, 'json', machineType, fname), {
                        encoding: 'utf-8',
                    });
                    ...
                    if (variant) {
                        ...
                        if (!fs.existsSync(path.join(__dirname, 'json', machineType, 'variants', variantName))) {
                            return g;
                        }
                        try {
                            const vFile = fs.readFileSync(
                                path.join(__dirname, 'json', machineType, 'variants', variantName),
                                'utf-8',
                            );
        ...
        case CodeTypes.M:
            {
                try {
                    const mFile = fs.readFileSync(path.join(__dirname, 'json', machineType, fname), {
                        encoding: 'utf-8',
                    });
